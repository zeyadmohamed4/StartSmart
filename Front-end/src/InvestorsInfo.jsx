...
function InvestorsInfo() {
  ...
  // Dummy data generator for investors only
  const generateDummyInvestors = (count = 10) => {
  ...
  // Fetch investors from API with fallback to dummy data
  const fetchInvestors = async () => {
    setLoading(true);
    setError(null);

    try {
      // Replace with your actual API endpoint for investors
      const response = await axios.get('https://localhost:7010/api/Users/GetInvestors', {
        headers: {
      ...
      // Ensure we always set an array of investors
      setInvestors(Array.isArray(response?.data) ? response.data : generateDummyInvestors());
    } catch (err) {
      console.error("Error fetching investors:", err);
      setError("Failed to load investors. Showing sample data.");
      setInvestors(generateDummyInvestors());
    } finally {
      setLoading(false);
    }
  };
  ...
  const deleteInvestor = async (userName) => {
    try {
      ...
      <div className="w-full flex">
        <div className="flex w-full">
          ...
          <div className="bg-white rounded-xl p-6 shadow-md w-full overflow-x-auto font-monst bg-gray-50">
            <h2 className="text-xl font-bold text-[#00192f] text-left my-6 font-monst font-[1000] mx-6">Investors</h2>
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full max-w-6xl mx-auto bg-gray-100 text-gray-700 text-sm shadow-lg rounded-lg mb-4">
                  <thead className="text-[14px] text-center bg-gray-300">
                    <tr>
                      <th className="py-3 px-4 border-b">Investor Name</th>
                  ...
                  <tbody>
                    {filteredInvestors.length > 0 ? (
                      filteredInvestors.map((investor) => (
                        <tr key={investor.id} className="text-center hover:bg-gray-200 transition-colors">
                          <td className="py-3 px-4 border-b">{investor.name}</td>
                          <td className="py-3 px-4 border-b">{investor.phoneNumber}</td>
                          <td className="py-3 px-4 border-b">{investor.email}</td>
                          <td className="py-3 px-4 border-b">{investor.country}</td>
                          <td className="py-3 px-4 border-b">{investor.city}</td>
                          ...
                          <td className="py-3 px-4 border-b">
                            <button
                              onClick={() => deleteInvestor(investor.userName)}
