...
function AllUsersInfo() {
  ...
  const generateDummyUsers = (count = 15) => {
  ...
  // Fetch users from API with fallback to dummy data
  const fetchUsers = async () => {
    ...
    try {
      ...
      const response = await axios.get('https://localhost:7010/api/Users/GetUsers', {
        headers: {
      ...
      <div className="w-full flex">
        <div className="flex w-full">
          ...
          <div className=" rounded-xl p-6 shadow-md w-full overflow-x-auto font-monst bg-gray-50">
            <h2 className="text-xl  text-[#00192f] text-left my-6 font-monst font-[1000] mx-6">All Users</h2>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center mb-6 px-6 gap-4">
              <div className="relative w-full">
                ...
                <input
                  ...
                  className="pl-10 pr-4 py-2 w-full border border-gray-400 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#4A9CCD] text-black font-monst rounded-xl text-[17px] placeholder:text-[17px]"
                  value={searchTerm}
                  ...
                  <tbody>
                    {filteredUsers.length > 0 ? (
                      filteredUsers.map((user) => (
                        <tr key={user.id} className="text-center hover:bg-gray-200 transition-colors">
                          <td className="py-3 px-4 border-b">{user.name}</td>
                          <td className="py-3 px-4 border-b">{user.phoneNumber}</td>
                          <td className="py-3 px-4 border-b">{user.email}</td>
                          <td className="py-3 px-4 border-b">{user.country}</td>
                          <td className="py-3 px-4 border-b">{user.city}</td>
                          <td className="py-3 px-4 border-b">
                            <span className={`px-2 py-1 rounded-full text-xs ${user.role === 'Owner' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'}`}>
                              {user.role}
                            </span>
                          </td>
                          <td className="py-3 px-4 border-b">
